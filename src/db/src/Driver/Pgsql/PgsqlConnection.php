 ...
 * Pgsql connection
 *
 * @Connection(driver=Driver::PGSQL)
 */
class PgsqlConnection extends AbstractDbConnection
{
    ...
    public function execute(array $params = [])
    {
        ...
        if ($result === false) {
            App::error('Pgsql execute error，connectError=' . $this->connection->connect_error . ' error=' . $this->connection->error);
        }
    ...
    {
        if (!$this->recv) {
            throw new PgsqlException('You forget to getResult() before rollback !');
        }
    ...
    {
        if (!$this->recv) {
            throw new PgsqlException('You forget to getResult() before commit !');
        }
    ...
    public function createConnection()
    {
        ...
        // error
        if ($Pgsql->connected === false) {
            throw new PgsqlException('Database connection error，error=' . $Pgsql->connect_error);
        }
    ...
    {
        $this->connection->query(sprintf('\c %s', $db));
        $this->currentDb = $db;
    }
